...
const page2 = (props) => {
  ...
  console.log(props.currentDataBigData);
  ...
  return (
    <>
      ...
      <CategoriesPage bigData={props.bigData} data={props.data}
      // givenGuid={givenGuid}
      />
      <ContactUsPage />
...
page2.getInitialProps = async ({query}) => {
  ...
  const currentData = await axios.get(`http://185.94.97.164/api/Category/${slug[1]}`)
  const currentDataBigData = await currentData.data
  // const finalcurrentDataData = await currentData.data.primaryCategories

  return { data, bigData, currentDataBigData }
}
